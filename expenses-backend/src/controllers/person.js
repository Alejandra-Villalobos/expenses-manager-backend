...
module.exports.registerPerson = async (req, res, next) => {
  ...
  try {
    ...
    const args = { name, email, password: password_hash };
...
module.exports.loginPerson = async (req, res, next) => {
  ...
  try {
    ...
    if (rows.length) {
      const { person, name, password: password_hash, email } = rows[0];
      const password_is_valid = await bcryptjs.compare(password, password_hash);
      if (password_is_valid) {
        const data_person = { person, name, email };
...
module.exports.infoPerson = async (req, res, next) => {
  const { person, name, email } = req.person;
  res.status(200).json({ data: [{ person, name, email }] });
};